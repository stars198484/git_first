...
public abstract class Figure implements Movable {
    ...
    public static final Figure getFigure(final int x, final int y) {
        Figure figure = null;
        ...
        if (figure == null)
            System.out.println("Warning: Can`t find figure with such coordinates");
        return figure;
    }

    public static final Figure getFigure(final String line) {
        int x = CoordinatesConverter.convertXtoInt(line);
        int y = CoordinatesConverter.convertYtoInt(line);

        return getFigure(x, y);
    }

    @Override
    public abstract boolean isRightMove(final String from, final String to);
}
