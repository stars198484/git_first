...
public class Pawn extends Figure implements Chessmen {
    ...
    public boolean isMoveTo(int xMove, int yMove, Chessboard cb) {
        ...
        // ход по диагонали, съесть противника
        if (cb.getFigure(xMove,yMove)==null || cb.getFigure(xMove,yMove).isWhiteFigure()==this.isWhiteFigure) return false;
        if (xMove == X + 1 & yMove == Y + 1) return true;
        if (xMove == X - 1 & yMove == Y - 1) return true;
        if (xMove == X + 1 & yMove == Y - 1) return true;
        if (xMove == X - 1 & yMove == Y + 1) return true;
